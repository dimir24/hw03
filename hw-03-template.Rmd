...
glance(emailhr_fit)$r.squared
```
Equation of line is y=0.6854031x-2.7573196. The R^2 value of the line is 0.0291194 which is low so the visualisation is not effective.
...
Using logistic regression because Agree and Not Agree are not numerical values, the are categorical values so logistic regression must be used.
...
gss16_rec_2 <- recipe(advfront~.,data=gss16_train) %>%
  step_rm(emailhr)%>%
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_zv(all_predictors())
...
Model 2 performs better as more data is being taken into account. Therefore the model will represent more of the data whereas model 1 can only represent one part of the data.
```{r}
